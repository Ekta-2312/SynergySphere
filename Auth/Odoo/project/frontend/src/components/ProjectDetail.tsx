...
export const ProjectDetail: React.FC<ProjectDetailProps> = ({
  ...
  const fetchProjectData = async () => {
    setLoading(true);
    try {
      await Promise.all([fetchTasks(), fetchDiscussions()]);
    } catch (error) {
  ...
  const handleTaskUpdate = (updatedTask: Task) => {
    setTasks(prev => {
      const updatedTasks = prev.map(task => (task._id === updatedTask._id ? updatedTask : task));
  ...
  const updateProjectStats = (currentTasks: Task[]) => {
    ...
    const completionPercentage = calculateProgressPercentage(currentTasks);
      ...
      <div className='flex-1 overflow-auto'>
        {loading ? (
          ...
          <>
            ...
            {activeTab === 'members' && (
              <MemberManagement project={project} onProjectUpdate={onProjectUpdate} />
            )}

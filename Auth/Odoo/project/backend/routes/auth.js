...
router.put('/profile/password', jwtAuth, controller.changePassword);
router.get('/profile/notifications', jwtAuth, controller.getNotificationSettings);
router.put('/profile/notifications', jwtAuth, controller.updateNotificationSettings);
  ...
  async (req, res) => {
    try {
      ...
      if (invitationToken) {
        ...
        const invitation = await ProjectInvitation.findOne({ token: invitationToken }).populate(
          'project',
        ...
        if (invitation && invitation.isValid()) {
          ...
          if (req.user.email === invitation.inviteeEmail) {
            ...
            return res.redirect(
              `${process.env.FRONTEND_URL || 'http://localhost:5173'}/auth/callback?token=${token}&user=${userData}&invitation=accepted`
            );
          } else {
            console.log('Email mismatch for invitation');
            return res.redirect(
              `${process.env.FRONTEND_URL || 'http://localhost:5173'}/invite/${invitationToken}?error=email_mismatch`
            );
          }
        } else {
          console.log('Invalid or expired invitation');
          return res.redirect(
            `${process.env.FRONTEND_URL || 'http://localhost:5173'}/invite/${invitationToken}?error=invalid_invitation`
          );
        }
      }
      ...
      res.redirect(
        `${process.env.FRONTEND_URL || 'http://localhost:5173'}/auth/callback?token=${token}&user=${userData}`
      );
    } catch (error) {
